...
describe('Lookup e2e test', () => {
  ...
  it('should load Lookups', async () => {
    ...
    expect(await lookupComponentsPage.getTitle()).to.eq('ensinoApp.lookup.home.title');
  });
  ...
  it('should load create Lookup page', async () => {
    ...
    expect(await lookupUpdatePage.getPageTitle()).to.eq('ensinoApp.lookup.home.createOrEditLabel');
  ...
  it('should create and save Lookups', async () => {
    ...
    await lookupComponentsPage.clickOnCreateButton();
    await promise.all([lookupUpdatePage.setNomeInput('nome'), lookupUpdatePage.setEntidadeInput('entidade')]);
    expect(await lookupUpdatePage.getNomeInput()).to.eq('nome', 'Expected Nome value to be equals to nome');
    expect(await lookupUpdatePage.getEntidadeInput()).to.eq('entidade', 'Expected Entidade value to be equals to entidade');
  ...
  it('should delete last Lookup', async () => {
    ...
    expect(await lookupDeleteDialog.getDialogTitle()).to.eq('ensinoApp.lookup.delete.question');
