...
import com.ravunana.ensino.service.MoradaPessoaService;
...
public class MoradaPessoaResourceIT {
     ...
     *
     * @see com.ravunana.ensino.repository.search.MoradaPessoaSearchRepositoryMockConfiguration
     */
    @Autowired
    ...
    public void createMoradaPessoa() throws Exception {
        ...
        // Validate the MoradaPessoa in Elasticsearch
        verify(mockMoradaPessoaSearchRepository, times(1)).save(testMoradaPessoa);
    }
    ...
    public void createMoradaPessoaWithExistingId() throws Exception {
        ...
        // Validate the MoradaPessoa in Elasticsearch
        verify(mockMoradaPessoaSearchRepository, times(0)).save(moradaPessoa);
    }
    ...
    public void checkProvinciaIsRequired() throws Exception {
        ...
        // set the field null
        moradaPessoa.setProvincia(null);
    ...
    public void checkMunicipioIsRequired() throws Exception {
        int databaseSizeBeforeTest = moradaPessoaRepository.findAll().size();
        // set the field null
        moradaPessoa.setMunicipio(null);
    ...
    public void deleteMoradaPessoa() throws Exception {
        ...
        // Validate the MoradaPessoa in Elasticsearch
        verify(mockMoradaPessoaSearchRepository, times(1)).deleteById(moradaPessoa.getId());
    }

    @Test
    @Transactional
    public void searchMoradaPessoa() throws Exception {
        // Initialize the database
        moradaPessoaRepository.saveAndFlush(moradaPessoa);
        when(mockMoradaPessoaSearchRepository.search(queryStringQuery("id:" + moradaPessoa.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(moradaPessoa), PageRequest.of(0, 1), 1));
        // Search the moradaPessoa
        restMoradaPessoaMockMvc.perform(get("/api/_search/morada-pessoas?query=id:" + moradaPessoa.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(moradaPessoa.getId().intValue())))
            .andExpect(jsonPath("$.[*].provincia").value(hasItem(DEFAULT_PROVINCIA)))
