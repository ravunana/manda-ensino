...
import com.ravunana.ensino.service.RelacionamentoPessoaService;
...
public class RelacionamentoPessoaResourceIT {
     ...
     *
     * @see com.ravunana.ensino.repository.search.RelacionamentoPessoaSearchRepositoryMockConfiguration
     */
    @Autowired
    private RelacionamentoPessoaSearchRepository mockRelacionamentoPessoaSearchRepository;
    ...
    public void createRelacionamentoPessoa() throws Exception {
        ...
        // Validate the RelacionamentoPessoa in Elasticsearch
        verify(mockRelacionamentoPessoaSearchRepository, times(1)).save(testRelacionamentoPessoa);
    }
    ...
    public void createRelacionamentoPessoaWithExistingId() throws Exception {
        ...
        // Validate the RelacionamentoPessoa in Elasticsearch
        verify(mockRelacionamentoPessoaSearchRepository, times(0)).save(relacionamentoPessoa);
    }
    ...
    public void deleteRelacionamentoPessoa() throws Exception {
        ...
        // Validate the RelacionamentoPessoa in Elasticsearch
        verify(mockRelacionamentoPessoaSearchRepository, times(1)).deleteById(relacionamentoPessoa.getId());
    }

    @Test
    @Transactional
    public void searchRelacionamentoPessoa() throws Exception {
        // Initialize the database
        relacionamentoPessoaRepository.saveAndFlush(relacionamentoPessoa);
        when(mockRelacionamentoPessoaSearchRepository.search(queryStringQuery("id:" + relacionamentoPessoa.getId()), PageRequest.of(0, 20)))
            .thenReturn(new PageImpl<>(Collections.singletonList(relacionamentoPessoa), PageRequest.of(0, 1), 1));
        // Search the relacionamentoPessoa
        restRelacionamentoPessoaMockMvc.perform(get("/api/_search/relacionamento-pessoas?query=id:" + relacionamentoPessoa.getId()))
            .andExpect(status().isOk())
            .andExpect(content().contentType(MediaType.APPLICATION_JSON_UTF8_VALUE))
            .andExpect(jsonPath("$.[*].id").value(hasItem(relacionamentoPessoa.getId().intValue())))
            .andExpect(jsonPath("$.[*].grauParentesco").value(hasItem(DEFAULT_GRAU_PARENTESCO)));
