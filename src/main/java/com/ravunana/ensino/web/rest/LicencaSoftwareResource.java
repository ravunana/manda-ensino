...
/**
 * REST controller for managing {@link com.ravunana.ensino.domain.LicencaSoftware}.
 */
@RestController
@RequestMapping("/api")
public class LicencaSoftwareResource {
    ...
    /**
     * {@code SEARCH  /_search/licenca-softwares?query=:query} : search for the licencaSoftware corresponding
     * to the query.
     *
     * @param query the query of the licencaSoftware search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/licenca-softwares")
    public ResponseEntity<List<LicencaSoftwareDTO>> searchLicencaSoftwares(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of LicencaSoftwares for query {}", query);
        Page<LicencaSoftwareDTO> page = licencaSoftwareService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
