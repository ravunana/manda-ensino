...
/**
 * REST controller for managing {@link com.ravunana.ensino.domain.RelacionamentoPessoa}.
 */
@RestController
@RequestMapping("/api")
public class RelacionamentoPessoaResource {
    ...
    /**
     * {@code SEARCH  /_search/relacionamento-pessoas?query=:query} : search for the relacionamentoPessoa corresponding
     * to the query.
     *
     * @param query the query of the relacionamentoPessoa search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/relacionamento-pessoas")
    public ResponseEntity<List<RelacionamentoPessoaDTO>> searchRelacionamentoPessoas(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of RelacionamentoPessoas for query {}", query);
        Page<RelacionamentoPessoaDTO> page = relacionamentoPessoaService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
