...
/**
 * REST controller for managing {@link com.ravunana.ensino.domain.ContactoPessoa}.
 */
@RestController
@RequestMapping("/api")
public class ContactoPessoaResource {
    ...
    /**
     * {@code SEARCH  /_search/contacto-pessoas?query=:query} : search for the contactoPessoa corresponding
     * to the query.
     *
     * @param query the query of the contactoPessoa search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/contacto-pessoas")
    public ResponseEntity<List<ContactoPessoaDTO>> searchContactoPessoas(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of ContactoPessoas for query {}", query);
        Page<ContactoPessoaDTO> page = contactoPessoaService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
