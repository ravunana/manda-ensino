...
/**
 * REST controller for managing {@link com.ravunana.ensino.domain.MoradaPessoa}.
 */
@RestController
@RequestMapping("/api")
public class MoradaPessoaResource {
    ...
    /**
     * {@code SEARCH  /_search/morada-pessoas?query=:query} : search for the moradaPessoa corresponding
     * to the query.
     *
     * @param query the query of the moradaPessoa search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/morada-pessoas")
    public ResponseEntity<List<MoradaPessoaDTO>> searchMoradaPessoas(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of MoradaPessoas for query {}", query);
        Page<MoradaPessoaDTO> page = moradaPessoaService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
