...
/**
 * REST controller for managing {@link com.ravunana.ensino.domain.Arquivo}.
 */
@RestController
@RequestMapping("/api")
public class ArquivoResource {
    ...
    public ResponseEntity<List<ArquivoDTO>> searchArquivos(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of Arquivos for query {}", query);
        Page<ArquivoDTO> page = arquivoService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
