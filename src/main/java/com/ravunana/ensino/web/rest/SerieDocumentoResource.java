...
/**
 * REST controller for managing {@link com.ravunana.ensino.domain.SerieDocumento}.
 */
@RestController
@RequestMapping("/api")
public class SerieDocumentoResource {
    ...
    public ResponseEntity<SerieDocumentoDTO> createSerieDocumento(@RequestBody SerieDocumentoDTO serieDocumentoDTO) throws URISyntaxException {
    ...
    public ResponseEntity<SerieDocumentoDTO> updateSerieDocumento(@RequestBody SerieDocumentoDTO serieDocumentoDTO) throws URISyntaxException {
    ...
    /**
     * {@code SEARCH  /_search/serie-documentos?query=:query} : search for the serieDocumento corresponding
     * to the query.
     *
     * @param query the query of the serieDocumento search.
     * @param pageable the pagination information.
     * @return the result of the search.
     */
    @GetMapping("/_search/serie-documentos")
    public ResponseEntity<List<SerieDocumentoDTO>> searchSerieDocumentos(@RequestParam String query, Pageable pageable) {
        log.debug("REST request to search for a page of SerieDocumentos for query {}", query);
        Page<SerieDocumentoDTO> page = serieDocumentoService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
