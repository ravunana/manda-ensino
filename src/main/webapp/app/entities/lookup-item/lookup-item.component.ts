...
export class LookupItemComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.lookupItemService
        ...
        .subscribe(
          (res: HttpResponse<ILookupItem[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  trackId(index: number, item: ILookupItem): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInLookupItems(): void {
    this.eventSubscriber = this.eventManager.subscribe('lookupItemListModification', () => this.loadPage());
  }

  delete(lookupItem: ILookupItem): void {
  ...
  protected onSuccess(data: ILookupItem[] | null, headers: HttpHeaders, page: number): void {
