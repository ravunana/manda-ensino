...
export class ContratoComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.contratoService
        ...
        .subscribe(
          (res: HttpResponse<IContrato[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInContratoes(): void {
    this.eventSubscriber = this.eventManager.subscribe('contratoListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: IContrato[] | null, headers: HttpHeaders, page: number): void {
