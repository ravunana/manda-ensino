...
export class RelacionamentoPessoaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.relacionamentoPessoaService
        ...
        .subscribe(
          (res: HttpResponse<IRelacionamentoPessoa[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  trackId(index: number, item: IRelacionamentoPessoa): number {
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-assertion
    return item.id!;
  }

  registerChangeInRelacionamentoPessoas(): void {
    this.eventSubscriber = this.eventManager.subscribe('relacionamentoPessoaListModification', () => this.loadPage());
  }

  delete(relacionamentoPessoa: IRelacionamentoPessoa): void {
  ...
  protected onSuccess(data: IRelacionamentoPessoa[] | null, headers: HttpHeaders, page: number): void {
    ...
    this.relacionamentoPessoas = data ? data : [];
  }
