...
export class NotaComponent implements OnInit, OnDestroy {
  ...
  loadPage(page?: number): void {
    ...
    if (this.currentSearch) {
      this.notaService
        ...
        .subscribe(
          (res: HttpResponse<INota[]>) => this.onSuccess(res.body, res.headers, pageToLoad),
          () => this.onError()
        );
      return;
    }
  ...
  registerChangeInNotas(): void {
    this.eventSubscriber = this.eventManager.subscribe('notaListModification', () => this.loadPage());
  }
  ...
  protected onSuccess(data: INota[] | null, headers: HttpHeaders, page: number): void {
